...
import pandas as pd #for use with data 
from bs4 import BeautifulSoup #for scraping purposes
import requests #for sending requests to webpages for scraping

url = "https://www.buyrentkenya.com/houses-for-rent" #site to be scraped
agent = "Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:122.0) Gecko/20100101 Firefox/122.0" #masks the request to human, so get 200 instead of 400 response from the server
...
#converting to a soup object the response
soup = BeautifulSoup(response.content,'html.parser')

#initialising empty lists that will contain the target details we need from the site
titles = []
locations = []
no_of_bathrooms = []
no_of_bedrooms = []
...
#isolating the listing-card and extracting the information we need from each
